...
def test_generate_list_view_for_DELETE_gets_forbidden_if_not_allowed():
    # Given a restroom API instance
    api = API()
    ...
    request = Mock(method='DELETE')
    response_from_DELETE = (api.generate_list_view("table_mymodel")
                            .as_view()(request, _id))
...
def test_generate_list_view_for_DELETE_gets_forbidden_even_if_allowed():
    # Given a restroom API instance
    api = API()
...
def test_generate_single_item_view_for_POST_gets_forbidden_if_not_allowed():
    # Given a restroom API instance
    api = API()
    ...
    request = Mock(method='POST')
    request.POST = QueryDict('text=Shalom')
    response_from_POST = (api.generate_single_item_view("table_mymodel")
                          .as_view()(request))
...
def test_generate_single_item_view_for_POST_gets_forbidden_even_if_allowed():
    # Given a restroom API instance
    api = API()
...
def test_generate_single_item_view_for_PUT():
    # Given a restroom API instance
    api = API()
    ...
    response_from_PUT = (api.generate_single_item_view("table_mymodel")
                         .as_view()(request, returned_obj.id))
    expected_response_json = {'id': 1, 'text': 'Hello'}
    response_json = ast.literal_eval(response_from_PUT.content)
    expect(response_json).to.equal(expected_response_json)


def test_generate_single_item_view_for_PUT_not_allowed_gets_forbidden():
    # Given a restroom API instance
    api = API()
